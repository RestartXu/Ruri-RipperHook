...
**GUI 虚拟文件预览**：MainForm「Load CABMap」→ 载 map + 自动载同名 `.names` sidecar → 开 `CabFileBrowser`（虚拟模式 ListView，每个 CAB 一行"虚拟文件"，列 Name/Container 路径/Source.chk/Types/Deps，258k 行后台线程物化+排序）。正则搜索框按 Container 路径/CAB/source/type 过滤（搜 "pelica"）。多选 = ListView 内置 ctrl/shift/跨行；**Ctrl+A 全选走 native `LVM_SETITEMSTATE`（iItem=-1）**——虚拟模式无托管全选，`SelectedIndices.Add` 循环是 O(n²)。右键「Load selected」= `MainForm.LoadCabsScopedAsync`（bundle-granular 载入预览，append 跨多次需累积 `_scopedLoadFilter`，否则 reload 旧+新会把旧 bundle 过滤掉）；「Export with dependencies」= `ExportCabsWithDepsAsync`（`ResolveScopedClosure` → 设 `LoadIncludeFile` → 复用 `RunFilteredExportAsync` 真导出）。GUI 侧 `ExportCabMap` 加 `LoadNames`/`EnumerateCabRows`/`ResolveScopedClosure`。**没载 .names sidecar 则做不了 bundle-granular**（闭包条目名为空 → `LoadIncludeFile` 留 null → 整块加载），大游戏会 OOM——所以 EndField 必须先 `--build-name-index`。
